...
Timing comes from the event markers embedded in the XDF recording. A single,
explicit marker-time correction is applied to every task and rest onset before
epoching; by default it is measured from each recording rather than typed in
(see :mod:`shared.marker_shift`). The signal pipeline follows the existing
``../emg_om_rms.py`` reference: volts to millivolts, causal 50 Hz notch, causal
20--150 Hz Butterworth band-pass, Teager--Kaiser energy, and a 100 ms window /
20 ms step envelope.
...
class ProcessingConfig:
    ...
    left_channels: tuple[str, str] = ("Aux 1.1", "Aux 1.2")
    right_channels: tuple[str, str] = ("Aux 2.1", "Aux 2.2")
    ...
    def validate(self) -> None:
        ...
        if len(self.left_channels) != 2 or len(self.right_channels) != 2:
            raise ValueError("each hand needs exactly two channels for bipolar EMG")
...
def recording_path(data_dir: str, name: str) -> Path:
...
def load_xdf(path: Path, requested: list[str], config: ProcessingConfig) \
    ...
    return LoadedRecording(signals, fs, duration, events, video)


def _resample_loaded(recording: LoadedRecording, target_fs: float | None) \
        -> LoadedRecording:
    if not target_fs or abs(recording.fs - target_fs) < 1e-9:
        return recording
    ratio = Fraction(target_fs / recording.fs).limit_denominator(1000)
    recording.signals = {
...
def _events_for(recording: LoadedRecording, config: ProcessingConfig,
    ...
    for condition in (config.left_condition, config.right_condition, config.rest_condition):
...
def analyze_recording(path: Path, name: str, config: ProcessingConfig,
    ...
    requested = list(dict.fromkeys(config.left_channels + config.right_channels))
    ...
    results = []
    for hand, condition, pair in hand_specs:
        ...
        if metrics is not None and metrics.rest_unreliable:
            note = (f"rest itself contains peaks ({metrics.rest_fpr:.0%} of "
                    f"{metrics.n_rest_windows} rest windows); the threshold is "
                    f"lifted by them and this count is an underestimate")
        elif metrics_error:
...
def _write_summary(path: Path, recordings: list[RecordingResult]) -> None:
    with open(path, "w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow([
            ...
            "timing", "figure", "trial_figures", "trial_metrics_csv",
        ])
        for recording in recordings:
            for hand in recording.hands:
                writer.writerow([
                    ...
                    hand.n_trials, hand.movement_duration_s, hand.trial_tail_ms,
                    hand.rest_baseline,
                    ...
                    recording.timing_note, hand.figure_path, hand.trial_dir,
        ...
        for hand in recording.hands:
            if hand.n_high_trials is None:
                lines.append(f"  {recording.recording:16s}{hand.hand:6s}"
                             f"{'not scored':>12s}")
                continue
            share = f"{hand.n_high_trials}/{hand.n_trials} = {hand.high_percent:.0f}%"
...
def planned_output_dir(config: ProcessingConfig) -> Path:
    ...
    return resolve_output_root(config.output_root) / participant
...
def analyze_batch(config: ProcessingConfig,
    ...
    for index, name in enumerate(config.recordings, start=1):
        path = recording_path(config.data_dir, name)
