...
from shared.recordings import classify_recording
...
class AnalysisWorker(QtCore.QThread):
    ...
    def run(self):
        try:
            ...
            self.done.emit(core.analyze_batch(
                self.config, self.progress.emit, require_metrics=False))
        except Exception:
...
class RecordingCard(QtWidgets.QGroupBox):
    ...
    def __init__(self, result, config, parent=None):
        ...
        self.result = result
        self.config = config
        condition = classify_recording(result.recording)
...
class EMGTab(QtWidgets.QWidget):
    ...
    The tab owns no input fields. Every setting it uses comes from the start
    tab and the settings file, so the EEG and EMG halves of a run are described
    by one set of participant details rather than two that can disagree.
    """
    ...
    # True when the batch was analysed, False when it failed. Emitted either
    # way: the EEG half waits on this, and must not be stranded by a failure.
    ...
    def _build(self):
        ...
        self.lbl_status = QtWidgets.QLabel(
            "Press START on the Start tab to analyse EMG and EEG together.")
    ...
    def _analysis_done(self, batch):
        ...
        self._set_status(
            ...
            f"{2 * len(batch.recordings)} hands scored. Nothing written yet.")
